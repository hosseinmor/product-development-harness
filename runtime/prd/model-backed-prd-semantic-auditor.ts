...
Treat each question's whyMaterial as proposed Product Child rationale, not as Product authority, evidence, or fact. Do not accept a claimed consequence merely because whyMaterial says the decision changes relevance, priorities, acceptance criteria, or another downstream concern. Verify that linkage independently from the supplied PM Intent, established current PRD behavior and scope, actually revealed human decisions, or valid current Product context. A PRD label or open-decision rationale that merely repeats the unsupported proposal does not establish the linkage. If the supplied authority and context do not support it, do not assume the consequence.

Choosing a Business Outcome, Product Goal, or success metric is not by itself a BLOCKING_PRODUCT_DECISION. Classify it as blocking only when the supplied authority establishes that leaving it unanswered prevents defining this capability's current scope, or leaves materially different core behavior, eligibility, lifecycle, required Product effects, or acceptance semantics for Design to invent. When it mainly guides measurement, optimization, ranking or tuning, prioritization, or later success evaluation while the core v0 behavior remains definable, classify it as NON_BLOCKING_PRODUCT_UNCERTAINTY. Multiple plausible outcomes, or the possibility that an outcome may influence later Product decisions, does not alone make the question blocking.

For every Product-owned question, apply this counterfactual test: if the question remains unanswered but explicit and bounded, can Design still define the current core flow, scope, eligibility, lifecycle, and acceptance behavior without selecting a Product answer? If yes, the question is not blocking. Apply this test especially to goals, outcomes, and metrics.
